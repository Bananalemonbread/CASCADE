...
class GPT4oRustTestGenerator(Generator):
    ...
    def build_prompt(self, context):
        ...
        #This should only be done if we have no original tests, or don't need P1 and P4
        if self.no_original_tests:
            test_header += build_test_suite(context, primer=f"// start writing tests for {context['signature']['name']} here")
        else:
            test_header += build_tests(context, primer=f"// start writing tests for {context['signature']['name']} here")
        ...
        if len(enc.encode(prompt)) > self.max_prompt_tokens:
            code = "// CODE:\n\n" + build_context(context, doc=True, no_fields=True, no_other_method_docs=True, no_other_methods=True)
            prompt = code + test_header
    ...
    def extract_tests(self, new_tests, context, response, output_path):
        ...
        new_tests = self.try_to_fix(new_tests, response, context, output_path, build_test_suite if self.no_original_tests else build_context)
